...
void ParseOptionsLookup(int argc, char **argv, ProgramOptions& opt) {
  int verbose_flag = 0;
  int single_flag = 0;
  int strand_flag = 0;
  int pbam_flag = 0;
...
void BenchmarkLookup(KmerIndex& index, ProgramOptions& opt) {
  auto k = index.k;
  size_t found = 0;
  size_t notFound = 0;
  size_t totalHits = 0;
  ...
  {
    ScopedTimer st;
    std::vector<std::string> read_file = {opt.files[0]};
    ...
    while (parser.refill(rg)) {
      ...
      for (auto& rp : rg) {
        ...
        ++rn;
        auto& r1 = rp.seq;
        KmerIterator kit1(r1.c_str()), kit_end;
        //Kmer x = kit1->first;
        //Kmer xr = x.rep();
        //auto search = index.kmap.find(xr);
        //bool forward = (x==xr);
        for (size_t i = 0; kit1 != kit_end; ++kit1, ++i) {
          auto phits = index.getRefPos(kit1->first);
          //Kmer x = kit1->first;
          //Kmer xr = x.rep();
          //auto search = index.kmap.find(xr);
          //bool forward = (x==xr);
          ...
          if (!phits.empty()) {
            ...
            //auto& contig = index.dbGraph.contigs[val.contig];
            //for (auto& txp : contig.transcripts) {
...
void BenchmarkValidate(KmerIndex& index, ProgramOptions& opt) {
  auto k = index.k;
  size_t found = 0;
  size_t notFound = 0;
  size_t correctPosCntr = 0;
        ...
        for (size_t i = 0; kit1 != kit_end; ++kit1, ++i) {
          ...
          if (search != index.kmap.end()) {
            ...
            for (auto& txp : contig.transcripts) {
              auto tpos = txp.pos;
              auto txid = txp.trid;
              bool sense = txp.sense;
              if (index.target_names_[txid] == rp.name) {
                foundTxp = true;
                auto res = index.findPosition(txid, xr, val, 0);
                auto offset = (x == xr) ? 0 : 30;
                //auto ppos = (sense) ? tpos + val.getPos() : tpos + val.contig_length - (val.getPos() + 1);
                if (res.first == i + 1 ) {
                  correctPos = true;
                } else {
                  //found.push_back(ppos);
                  //truep.push_back(i);
                  //std::cerr << "pos = " << res.first << ", true pos = " << i + 1 << "\n";
                }
              }
            }
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    ...
    if (cmd == "version") {
      ...
      std::cerr << "HERE\n";
      ParseOptionsLookup(argc-1, argv+1, opt);
      KmerIndex index(opt);
      index.load(opt);
      BenchmarkLookup(index, opt);
    } else if (cmd == "validate") {
      std::cerr << "HERE\n";
      ParseOptionsLookup(argc-1, argv+1, opt);
      KmerIndex index(opt);
      index.load(opt);
      BenchmarkValidate(index, opt);
    } else if (cmd == "index") {
      cerr << endl;
      if (argc==2) {
        usageIndex();
