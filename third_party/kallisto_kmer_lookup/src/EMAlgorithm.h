...
struct EMAlgorithm {
    ...
    opt(opt)
  {
    ...
    std::ofstream ofile("KallistoEqClasses");
    size_t nnames{target_names_.size()};
    size_t nonEmptyEC{0};
    for (size_t ec = 0; ec < ecmap_.size(); ++ec) {
    ...
    std::cerr << "Number of non-empty equivalence classes = " << nonEmptyEC << '\n';

    ofile << nnames << "\n";
    ofile << nonEmptyEC << "\n";

    for (auto i = 0; i < nnames; ++i) {
        ofile << target_names_[i] << "\n";
    }
