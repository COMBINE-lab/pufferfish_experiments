...
void KmerIndex::load(ProgramOptions& opt, bool loadKmerTable) {
  ...
  std::cerr << "[index] reading in target names " << std::endl;
  ...
  std::cerr << "[index] reading in contig sequences " << std::endl;
  // 10. read contigs
  size_t contig_size;
  ...
  for (auto i = 0; i < contig_size; i++) {
    ...
    if (i % 1000000 == 0) { std::cerr << "[index] contig " << i << " of " << contig_size << std::endl; }
    Contig c;
    ...
    //memset(buffer,0,bufsz);
    in.read(buffer, tmp_size);
    // rather than memset the whole buffer, just make
    // sure that the bytes from which we want to build
    // c.seq are null-terminated.
    buffer[tmp_size-1] = '\0';
    ...
    for (auto j = 0; j < tmp_size; j++) {
      ...
      int trid, pos;
      bool sense;
      in.read((char*)&trid, sizeof(trid));
      in.read((char*)&pos, sizeof(pos));
      in.read((char*)&sense, sizeof(sense));
      c.transcripts.emplace_back(trid, pos, sense);
    }

    dbGraph.contigs.emplace_back(std::move(c));
  }

  std::cerr << "[index] reading in ec labels" << std::endl;
  // 11. read ecs info
  dbGraph.ecs.clear();
...
auto KmerIndex::getRefPos(Kmer& mer) -> ProjectedHit {

  using IterT = std::vector<ContigToTranscript>::iterator;
  auto km = mer.rep();
  ...
  if (search != kmap.end()) {
    /*
      ...
      */
    auto& kv = search->second;
      auto& txps = dbGraph.contigs[kv.contig].transcripts;
      return {kv._pos, forward, kv.contig_length, Kmer::k, core::range<IterT>{txps.begin(), txps.end()}};
  } else {
    return {std::numeric_limits<uint32_t>::max(), true, 0, Kmer::k, core::range<IterT>{}};
  }
