...
#define KMER_SZ 31
	...
	while (kseq_read(seq) >= 0) {
		...
		/*
		   kstring_t** kmerArray = (kstring_t**)malloc((seq->seq.l-KMER_SZ+1) * sizeof(kstring_t*)) ;

		   for(i = 0; i < seq->seq.l - KMER_SZ + 1; ++i){
		   kmerArray[i] = (kstring_t*)malloc(1 * sizeof(kstring_t));
		   kmerArray[i][0].l = KMER_SZ ;
		   kmerArray[i][0].s = (char*)malloc(KMER_SZ*sizeof(char)) ;
		   for(j = i ; j < i + KMER_SZ ; ++j){
		   kmerArray[i][0].s[j-i] = seq->seq.s[j] ;
		   }
		   }*/
		//smem_set_query(itr, seq->seq.l, (uint8_t*)seq->seq.s);
		//char* kmer = (char*)malloc(31);
